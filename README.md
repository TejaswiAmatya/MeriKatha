...
└── prisma/
    ...
    └── seed.ts               # Seed script — loads initial Nenglish stories into the DB
```
...
# Seed the database with sample Nenglish stories
npm run seed
...
### Nenglish Language Policy

All user-facing strings — error messages, Swagger descriptions, seed data — use Nenglish. No clinical mental health vocabulary anywhere in the codebase. This is enforced in `CLAUDE.md`.
...
- [ ] **Aangan Bot** — Claude-powered witness/validator bot (non-advisory, Nenglish only)
...
This backend is part of **MannSathi**, a women-first Nepali mental health platform built for Nepali women in Nepal and the diaspora. The full platform vision includes Meri Katha (this API), an Aangan Bot, a mood tracker, and Didi Circles.
